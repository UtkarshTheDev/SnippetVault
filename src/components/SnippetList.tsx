...
const SnippetList = React.forwardRef<HTMLDivElement, SnippetListProps>(
  ({ filters }, ref) => {
    ...
    const fetchSnippets = async () => {
      setLoading(true);
      const storedSnippets = await loadSnippets(filters);
      ...
      if (hasFilters) {
        return (
          <div className="flex flex-col items-center justify-center h-full p-8 text-center">
            <FilterX className="h-16 w-16 text-muted-foreground mb-4" />
